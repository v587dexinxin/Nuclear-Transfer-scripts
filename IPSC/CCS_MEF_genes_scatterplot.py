...
dataFolder = 'H:\\Workspace_New\\data\\IPSC_ATAC\\RNA\\gene_expression'
...
union = pd.read_table('H:\\Workspace_New\\data\\RNA\\gene_expression\\all_genes.txt' , index_col=1)

Merge = pd.concat([union[[u'Gene_id', u'Chr', u'Start', u'End', u'CCS_1_FPKM', u'CCS_2_FPKM', u'CCS_3_FPKM']],
...
Diff = pd.read_csv('H:\\Workspace_New\\data\\IPSC_ATAC\\RNA\\reads_count\\CCS_MEF.csv', index_col=1)           
...
for i in Merged.index:
    ccs = (Merged.ix[i].CCS_1_FPKM + Merged.ix[i].CCS_2_FPKM + Merged.ix[i].CCS_3_FPKM ) / 3 
    mef = (Merged.ix[i].MEF_1_FPKM + Merged.ix[i].MEF_2_FPKM ) / 2
    if ((Merged.ix[i].log2FoldChange) > 0 and (mef > ccs)) or ((Merged.ix[i].log2FoldChange) < 0 and (mef < ccs)):
        index_new.append(i)
        
 
Merged = Merged.ix[index_new]
