...
for i in A_B:
    ...
    if overlap.size > 0:
        for j in overlap:
            NT_c1.append(j)
NT_c1 = np.array(NT_c1 , dtype = union_type)  
...
for i in B_A:
    ...
    if overlap.size > 0:
        for j in overlap:
            NT_c2.append(j)
NT_c2 = np.array(NT_c2 , dtype = union_type)                 
...
for i in IPSC_stable:
    ...
    if overlap.size > 0:
        for j in overlap:
            c3.append(j)
c3 = np.array(c3 , dtype = union_type_2)
