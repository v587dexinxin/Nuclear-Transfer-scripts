# -*- coding: utf-8 -*-
"""
Created on Wed Oct 30 21:48:58 2019
...
Outfolder = 'D:\\ntESC_3Dreprogramming\\Figures\\Plot\\Fig2_figs\\Compartment'
...
for g in chrom:
    ...
    for i in range((len(tmp_CCS) // interval) + 1):
        ...
        ##PCA Tracks
        ax = add_ax(sig1 , [Left, HB , width , 0.1] , 'CCS')
        ...
        ax = add_ax(sig2 , [Left, HB + 0.1 , width , 0.1] , 'NT5')
        caxis_S_horizontal(ax)
        ax = add_ax(sig3 , [Left, HB + 0.2, width , 0.1] , 'NT6')
        caxis_S_horizontal(ax)
        ax = add_ax(sig4 , [Left, HB + 0.3, width , 0.1] , 'fESC')
        caxis_S_horizontal(ax)
