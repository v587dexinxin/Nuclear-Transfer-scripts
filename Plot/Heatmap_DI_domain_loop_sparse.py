# -*- coding: utf-8 -*-
"""
Created on Fri Nov 09 15:24:28 2018
...
OutFolder = '/public/home/xxli/data/BDF1_New/HiC/Plot'
LoopFolder = '/public/home/xxli/data/BDF1_New/HiC/Loop/Raw_10K'
...
for c in cell:
    OutFil = c + '_test_10K.pdf'
    pp = PdfPages(os.path.join(OutFolder , OutFil))
    HiCFolder = '/public/home/xxli/data/BDF1_New/HiC/HapHiC_workspace/' + c + '_workspace/' + c + '_10K'
    HiCFil = 'Correct_Merged_Reps_Local_Chromosome_Matrix.npz'
    HiCSource = os.path.join(HiCFolder , HiCFil)
    HiCData = np.load(HiCSource)
    LoopFil = 'Clustered_' + c + '_loops_10K.txt'
    LoopFil_raw = c + '_loops_10K.txt'
    ...
    for g in ['1']:
        ...
        for i in range(len(lib) // interval +1):
            startHiC = i*interval
            endHiC = (i+1)*interval
            matrix = getmatrix(lib , startHiC , endHiC)
            di = CalDI(matrix)
            sig = np.array(di)
            ...
            for p in extract:
                ...
                y = p['end'] // R - startHiC
                ax.scatter(y + 0.5 ,x + 0.5 , color = '', edgecolors = 'b', s = 15)
            for p in extract_raw:
                x = p['end'] // R - startHiC
                y = p['start'] // R - startHiC
                ax.scatter(y + 0.5,x + 0.5 , marker = 's' , color = 'b', edgecolors = '', s = 5)
            ...
            ##Signal Tracks
            ax = fig.add_axes([Left-0.1, HB, 0.1, HH])
