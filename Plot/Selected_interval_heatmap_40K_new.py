# -*- coding: utf-8 -*-
"""
Created on Mon Dec 03 15:45:19 2018
...
def caxis_H(ax):
    ...
    ax.tick_params(axis = 'both', labelsize = 18, length = 5, pad = 7)
...
size = (12, 12)
Left = 0.25 ; HB = 0.13 ; width = 0.5 ; HH = 0.5
...
pp1 = PdfPages('D:\\Workspace_New\\Plot\\Figure2\\new\\Selected_CCS_40K_Heatmap_stable_window_wise_680K.pdf')
pp2 = PdfPages('D:\\Workspace_New\\Plot\\Figure2\\new\\Selected_NT5_40K_Heatmap_stable_window_wise_680K.pdf')
pp3 = PdfPages('D:\\Workspace_New\\Plot\\Figure2\\new\\Selected_NT6_40K_Heatmap_stable_window_wise_680K.pdf')
pp4 = PdfPages('D:\\Workspace_New\\Plot\\Figure2\\new\\Selected_fESC_40K_Heatmap_stable_window_wise_680K.pdf')
...
for i in interval:
    ...
    for c in cell:
        ...
        union = union_loop[union_loop['chr'] == g]
        ...
        labelsx = [properU(p) for p in pos]
        labelsy = [properU(p) for p in pos[:3]]
...
#b3b3b3
        ...
        for p in extract:
            ...
            y = p['end'] // R - startHiC
            ax1.scatter(x + 0.5 ,y + 0.5 , color = '', edgecolors = 'b', s = 200)
#            dis = {}
#            for j in union:
#                d = (j['start'] - p['start']) ** 2 + (j['end'] - p['end']) ** 2
#                dis[d] = j
#                
#            strength = dis[min(dis.keys())][c]
#            ax1.text(y + 1.5 , x + 1 , str(round(strength,2)) , size = 15)
        ...
        caxis_H(ax1)
        ## Colorbar
        ax2 = fig.add_axes([Left + 0.44 , HB - 0.06 , 0.06 , 0.015])
        ...
        ax4 = fig.add_axes([Left, HB + width + 0.08 , width , 0.075])
        ...
        ax4.set_ylabel('PC1',fontsize=30,rotation = 'horizontal' , labelpad = 50)
        caxis_S(ax4)
        
        #=====================ATAC + RNA Signal================================
        #RNA
        location1 = [Left , HB + HH + 0.16 , width , 0.075]
        color = 'fuchsia'
        ...
        #ATAC
        location2 = [Left, HB + HH + 0.24, width, 0.075]
        color = 'blue'
