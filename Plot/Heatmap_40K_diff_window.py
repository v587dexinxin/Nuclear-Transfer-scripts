# -*- coding: utf-8 -*-
"""
Created on Wed Apr 24 22:10:08 2019
...
TadFolder = 'H:\\Workspace_New\\data\\HiC\\HiTAD\\respective\\stable_600K'
OutFolder = 'H:\\Workspace_New\\Plot\\Heatmap_40K_diff_window\\stable_600K'
...
for c in cell:
    HiCData = np.load('H:\\Workspace_New\\data\\HiC\\Matrix\\Matrix_40K\\' + c + '_40K\\Correct_Merged_Reps_Local_Chromosome_Matrix.npz' , allow_pickle=True)
    HiCData = HiCData['Matrix'][()]
    TadData = np.loadtxt(os.path.join(TadFolder , c + '_40K_allreps.txt') , usecols = (0 , 1 , 2) ,dtype = tad_type)      
    DIData = CalDI(HiCData)
    for g in chrom:
        ...
        for i in range(len(lib) // interval +1):
            ...
            for corner in pairs:
                if (corner[0] <= 0):
                    ax.plot([0, corner[1]], [corner[1], corner[1]], color = 'black',
                            linewidth = 1)
                    ax.plot([corner[1], corner[1]], [0, corner[1]], color = 'black',
                            linewidth = 1)
